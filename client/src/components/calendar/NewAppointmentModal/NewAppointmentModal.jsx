...
export function NewAppointmentModal({
    ...
    newAppt, setNewAppt,
    turnoEsPasado,
    clienteSeleccionado, clearClienteSeleccionado,
    clienteSearch, setClienteSearch,
    clientesFiltrados, selectCliente,
    autosDelCliente, servicios,
    addAppt, confirmDisabled, saving,
}) {
    return (
        <Modal
            open={showModal}
            onClose={() => setShowModal(false)}
            title={`Nuevo Turno — ${selD} de ${MONTHS[selM - 1]?.label}`}
            size="md"
            actions={
                <>
            ...
            <div className={styles.modalForm}>
                ...
                <div className={styles.inputGroup}>
                    ...
                    <input
                        ...
                        type="time"
                        value={newAppt.hora}
                        onChange={(e) => setNewAppt((p) => ({ ...p, hora: e.target.value }))}
                    />
                    {turnoEsPasado && (
                    ...
                    {clienteSeleccionado ? (
                        <div className={styles.selectedClientBox}>
                            <div className={styles.selectedClientInfo}>
                                <div className={styles.selectedClientName}>{clienteSeleccionado.nombre}</div>
                                <div className={styles.selectedClientMeta}>
                                    {[
                                        clienteSeleccionado.telefono,
                                        clienteSeleccionado.email,
                                        (clienteSeleccionado.Autos || []).map((a) => a.patente).filter(Boolean).join(" · "),
                                    ].filter(Boolean).join(" — ")}
                                </div>
                            </div>
                            <button type="button" className={styles.changeClientBtn} onClick={clearClienteSeleccionado}>
                                Cambiar
                            </button>
                        </div>
                    ) : (
                        <>
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    setNewAppt((p) => ({ ...p, cliente_id: "", auto_id: "" }));
                                }}
                            />
                            <div className={styles.clientSearchResults}>
                                {clientesFiltrados.length === 0 ? (
                                    <div className={styles.clientSearchEmpty}>No se encontraron clientes.</div>
                                ) : (
                                    clientesFiltrados.map((c) => (
                                        ...
                                        >
                                            <div className={styles.clientResultName}>{c.nombre}</div>
                                            <div className={styles.clientResultMeta}>
                                                {[
                                                    c.telefono, c.email,
                                                    (c.Autos || []).map((a) => a.patente).filter(Boolean).join(" · "),
                                                ].filter(Boolean).join(" — ")}
                                            </div>
                                        </button>
                                    ))
                ...
                <div className={styles.inputGroup}>
                    ...
                    <select
                        ...
                        onChange={(e) => setNewAppt((p) => ({ ...p, auto_id: e.target.value }))}
                ...
                <div className={styles.inputGroup}>
                    ...
                    <select
                        ...
                        onChange={(e) => setNewAppt((p) => ({ ...p, servicio_id: e.target.value }))}
                    >
